...
This project provides a Docker container for [PuTTY](https://www.putty.org).
...
services:
  putty:
    ...
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/putty:/config:rw"
```
...
[Releases]: https://github.com/jlesage/docker-putty/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/putty/tags
...
container, it would proxy all HTTP requests for `putty.domain.tld` to
...
server {
	...
	location /putty/ {
		...
		# Needed when audio support is enabled.
		location /putty/websockify-audio {
			proxy_pass http://docker-putty/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of PuTTY integrated into Docker image. |
...
Any desktop notification generated by PuTTY is forwarded to the
